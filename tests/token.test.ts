...
describe('TokenService', () => {
  ...
  it('round-trips a payload', async () => {
    const token = await service.generateToken({ userId: 'u1', sessionId: 's1' });
  ...
  it('rejects a token signed with a different secret', async () => {
    ...
    const token = await other.generateToken({ userId: 'u1', sessionId: 's1' });
  ...
  it('refuses to issue tokens with the insecure default secret', async () => {
    ...
    await expect(insecure.generateToken({ userId: 'u1', sessionId: 's1' })).rejects.toThrow(
